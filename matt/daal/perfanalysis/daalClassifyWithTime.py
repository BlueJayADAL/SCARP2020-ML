...
def Classify(workDir, select, classify, test, inFile, output, model, http, tls, dns):
	startTime = time.time()
...
#			filterOut = set(filterOut)
	...
	if workDir == None or not os.path.isdir(workDir):
	...
	if classify == False and test == False:
		print('At least test or classify')
		return
	if classify == True and test == True:
		print('Classify and Test cannot work at the same time')
		return
	if test == True and (inFile == None or os.path.isdir(inFile)):
		print('Test Needs Input Parameters File')
		return
	if classify == True and (output == None or os.path.isdir(output)):
	...
	if classify == True:
		...
		if select == None or not os.path.isfile(select):
			print('No valid selection JSON file')
			return

		#check if at least one of the features is enabled
		if dns == False and http == False and tls == False:
